...
class DancingLines: Program {
    var lines: [Line] = []

    override func setup() {
        framesPerSecond = 30
        let divisor = 2
        canvasSize = [1920 / divisor, 1080 / divisor]

        for _ in 1...5 {
            lines.append(Line(bounds: Rect(canvas.bounds), maxHistoryCount: 20))
        }
    ...
    override func update() {
        lines.forEach { line in
            line.update(bounds: Rect(canvas.bounds))
        }
    }
